...
<div class="px-2">
    ...
    @if ($currentStep <= 2) <h3 class="text-center text-xl my-4">Datos Alumno</h3>
        @endif
        @if ($currentStep === 3)
        ...
        <form class="flex flex-col gap-y-14 mt-6 items-center" wire:submit="submit">
            ...
            <div>
                <div class="flex flex-col md:flex-row md:flex-wrap justify-between gap-y-4 md:gap-y-8 w-full">
                    ...
                    <x-input type="text" id="telefono" label="Teléfono" placeholder="Introduce un telefono" wire:model="telefono" />
                </div>
            </div>
            @endif
            @if ($currentStep === 2)
            <div>
                <div class="flex flex-col md:flex-row md:flex-wrap justify-between gap-y-4 md:gap-y-8 w-full">
                    ...
                    <div class="w-[45%] flex gap-x-2">
                        <x-input type="number" id="numeracion" label="Numeración" placeholder="Numeración" wire:model="numeracion" />
                    ...
                    <x-input type="text" id="localidad" label="Localidad" placeholder="Localidad" wire:model="localidad" />
                    ...
                    <div class="md:max-w-[45%] w-full flex flex-col gap-y-2">
                        ...
                        <div class="w-full grid grid-cols-2 gap-2">
                            <x-input-check id="publico" label="Trasporte público" value="transporte-publico" wire:model="transporte" />
                            <x-input-check id="auto" label="Auto / Camioneta" value="auto-camioneta" wire:model="transporte" />
                    ...
                    <div class=" w-[45%] flex flex-col gap-y-2">
                        ...
                        <div class="flex md:max-w-[45%] w-full gap-6">
                            <x-input-radio id="obra-social" label="Si" value="si" wire:model.live="obraSocial" />
                            <x-input-radio id="obra-social" label="No" value="no" wire:model.live="obraSocial" />
                        </div>
                        <x-input type="text" id="nombre-os" label="" placeholder="Obra Social / Prepaga" wire:model="nombreObraSocial" :disabled="$obraSocial != 'si'" :value="$obraSocial == 'no' ? '' : $nombreObraSocial" />
                        @error('obraSocial')
                        <p class="text-red-700 text-sm">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div>
                <div class="flex flex-col md:flex-row md:flex-wrap justify-between gap-y-4 md:gap-y-8 w-full">
                    <x-input type="text" id="nombreTutor" label="Nombre" placeholder="Nombre" wire:model="nombreTutor" />
                    <x-input type="text" id="apellidoTutor" label="Apellido" placeholder="Apellido" wire:model="apellidoTutor" />
                    <x-input type="text" id="cuilTutor" label="CUIL" placeholder="Cuil sin guiones ni puntos" wire:model="cuilTutor" />
                    <x-input type="email" id="emailTutor" label="Email" placeholder="Introduce un correo" wire:model="emailTutor" />
                    <x-input type="text" id="telefonoTutor" label="Teléfono" placeholder="Introduce un telefono" wire:model="telefonoTutor" />
                    ...
                    <div class="md:max-w-[45%] w-full flex flex-col gap-y-2">
                        ...
                        <x-input-radio id="madre" name="parentezco" label="Madre" value="madre" wire:model="parentezco" />
                        <x-input-radio id="padre" name="parentezco" label="Padre" value="padre" wire:model="parentezco" />
                        <x-input-radio id="hermanos" name="parentezco" label="Hermano/a" value="hermanos" wire:model="parentezco" />
                        <x-input-radio id="tios" name="parentezco" label="Tia/o" value="tios" wire:model="nombreTuparentezcotor" />
                        <x-input-radio id="abuelos" name="parentezco" label="Abuela/o" value="abuelos" wire:model="parentezco" />
                        <x-input-radio id="otro" name="parentezco" label="Otro" value="otro" wire:model="parentezco" />
                    </div>
                </div>
                @error('parentezco')
                <p class="text-red-700 text-sm">{{ $message }}</p>
            ...
            <div>
                <div class="flex flex-col md:flex-row md:flex-wrap justify-between gap-y-4 md:gap-y-8 w-full">
                    ...
                    <x-input type="text" id="escuela-proviene" label="Escuela que proviene" placeholder="Nombre Escuela" wire:model="escuelaProviene" />
                    <div class="md:max-w-[45%] w-full flex flex-col gap-y-2">
                        <p class="text-[#2D3648] font-semibold text-sm">Turno</p>
                        <div class="flex md:max-w-[45%] w-full gap-6">
                            <x-input-radio id="maniana" label="Mañana" value="maniana" name="turno" wire:model="turno" />
                            <x-input-radio id="tarde" label="Tarde" value="tarde" name="turno" wire:model="turno" />
                        </div>
                    </div>
                    <div class="md:max-w-[45%] w-full flex flex-col gap-y-2">
                        ...
                        <div class="flex md:max-w-[45%] w-full gap-x-8">
                            <div class="flex flex-col gap-3">
                                <x-input-radio id="ingresante" label="Ingresante" value="ingresante" name="condicion-alumno" wire:model="condicionAlumno" />
                                <x-input-radio id="regular" label="Regular" value="regular" name="condicion-alumno" wire:model="condicionAlumno" />
                            </div>
                            <div class="flex flex-col gap-3">
                                <x-input-radio id="traspaso" label="Traspaso" value="traspaso" name="condicion-alumno" wire:model="condicionAlumno" />
                                <x-input-radio id="repitente" label="Repitente" value="repitente" name="condicion-alumno" wire:model="condicionAlumno" />
                            </div>
                        </div>
                    </div>
                    <div class="md:max-w-[45%] w-full flex flex-col gap-y-2">
                        ...
                        <div class="flex md:max-w-[45%] w-full gap-6">
                            <x-input-radio id="si" label="Si" value="si" name="adeuda-materia" wire:model.live="adeudaMaterias" />
                            <x-input-radio id="no" label="No" value="no" name="adeuda-materia" wire:model.live="adeudaMaterias" />
                        </div>
                        <div class="w-[220%]">
                            <x-input type="text" id="adeuda-materia-nombre" label="" placeholder="Nombres materias" wire:model="nombreMaterias" :disabled="$adeudaMaterias != 'si'" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="w-full flex flex-col gap-y-2">
                    <x-input-check id="familiar" label="Tengo un familiar que es alumno escuela" value="familiar" wire:model="reconocimientos" />
                    <x-input-check id="merito" label="Reconocimiento al mérito" value="merito" wire:model="reconocimientos" />
                    <x-input-check id="otros" label="Otros reconocimientos (concursos, mejor compañero,  etc)" value="otros" wire:model="reconocimientos" />
                ...
                <div class="w-full flex gap-2 justify-start items-center mt-2">
                    <input class="border border-gray-300 p-2 rounded h-5 w-5" id="terminos" name="terminos" type="checkbox" wire:model="terminos">
                    <label for="terminos">
                        He leído y acepto los términos del <span id="openModalBtn" class="italic underline hover:cursor-pointer">código de
                            vestimenta.</span>
                    </label>
                </div>
            ...
            <div class="flex gap-4 w-full justify-center">
                ...
                @endif
                @if ($currentStep
                < 5) <x-primary-button text="Siguiente" wire:click="incrementSteps" type="button" />
                @endif
                        ...
                        <div>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_96_3094)">
                                    <path d="M21.8715 0.905273H2.12965C1.45344 0.905273 0.905273 1.45344 0.905273 2.12965V21.8715C0.905273 22.5477 1.45344 23.0959 2.12965 23.0959H21.8715C22.5477 23.0959 23.0959 22.5477 23.0959 21.8715V2.12965C23.0959 1.45344 22.5477 0.905273 21.8715 0.905273Z" fill="#3D5A98" />
